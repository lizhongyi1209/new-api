		...
		if val, ok := request.Extra["parameters"]; ok {
			...
			if request.BillingParameters == nil {
				request.BillingParameters = &dto.ImageBillingParameters{
					N:            imageRequest.Parameters.N,
					PromptExtend: imageRequest.Parameters.PromptExtend,
				}
			}
		}
...
func aliImageHandler(a *Adaptor, c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*types.NewAPIError, *dto.Usage) {
	...
	if count < 0 || count > dto.MaxImageN {
		logger.LogWarn(c, fmt.Sprintf("invalid Ali image usage count %d; falling back to response images or requested quantity", count))
		count = 0
	}
	if count == 0 {
	...
	if count > dto.MaxImageN {
		logger.LogWarn(c, fmt.Sprintf("invalid Ali response image count %d; retaining requested quantity", count))
	} else if count > 0 {
