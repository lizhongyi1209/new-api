...
func (a *Adaptor) ConvertOpenAIRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.GeneralOpenAIRequest) (any, error) {
	...
	if info.ChannelType != constant.ChannelTypeOpenRouter && renderReasoning {
		intent, diagnostics, err := kitreasoning.FromOpenAIChat(request)
		if err != nil {
			return nil, kitreasoning.AsClientError(err)
		}
		recordReasoningDiagnostics(c, info, diagnostics)
		if err := kitreasoning.ApplyToOpenAIChat(request, intent); err != nil {
			return nil, kitreasoning.AsClientError(err)
		}
	...
	modelSupportsReasoning := dto.GetOpenAIChatCapabilities(originModel, request.ReasoningEffort).UseMaxCompletionTokens
	if modelSupportsReasoning {
		// Resolve the model suffix before evaluating parameter capabilities.
		if effort != "" {
