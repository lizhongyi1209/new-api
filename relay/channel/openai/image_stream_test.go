...
// disconnects after receiving that event.
...
func TestOpenaiImageHandlerUsesPositiveActualCountForFixedPrice(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "fixed price raises count when upstream returns more",
			body:      `{"data":[{},{},{},{}]}`,
			usePrice:  true,
			wantCount: 4,
		},
		...
		{
			name:      "count beyond billing bound keeps requested count",
			body:      `{"data":[` + strings.TrimSuffix(strings.Repeat(`{},`, dto.MaxImageN+1), ",") + `]}`,
			usePrice:  true,
			wantCount: 3,
		},
		{
