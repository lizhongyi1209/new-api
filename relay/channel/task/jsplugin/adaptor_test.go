...
func TestTaskAdaptorMapsJSContract(t *testing.T) {
	...
	queryResp, err := adaptor.FetchTaskWithContext(server.URL, "secret", &model.Task{
		Action:      info.Action,
	...
	result, err := adaptor.ParseTaskResultWithContext(&model.Task{}, queryResp, queryBody)
	require.NoError(t, err)
...
`
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			result, parseErr := adaptor.ParseTaskResultWithContext(&model.Task{}, &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)}, body)
	...
	t.Run("declared completion token unit is saturated instead of discarded", func(t *testing.T) {
		...
		require.NoError(t, err)
		result, err := adaptor.ParseTaskResultWithContext(&model.Task{}, &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)}, body)
		require.NoError(t, err)
...
`
	...
	result, err := adaptor.ParseTaskResultWithContext(&model.Task{}, &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)}, []byte(`{"completion":13,"total":17}`))
	require.NoError(t, err)
...
`
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			resp, fetchErr := adaptor.FetchTaskWithContext(server.URL, "secret", testCase.task, "")
...
`
	...
	resp, err := adaptor.FetchTaskWithContext(server.URL, "secret", task, "")
	require.NoError(t, err)
	require.NoError(t, resp.Body.Close())
	...
	result, err := adaptor.ParseTaskResultWithContext(task, &http.Response{StatusCode: http.StatusTeapot, Header: make(http.Header)}, []byte(`{"ok":true}`))
	require.NoError(t, err)
...
`
	...
	t.Run("completion selects each persisted model including legacy origin fallback", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			require.NoError(t, err)
			result, err := adaptor.ParseTaskResultWithContext(task, &http.Response{StatusCode: http.StatusOK}, body)
			require.NoError(t, err)
			if tc.accepted {
	...
	t.Run("legacy completion facts remain usable by stored expressions", func(t *testing.T) {
		...
		adaptor := New(updated)
		adaptor.Init(info)
		result, err := adaptor.ParseTaskResultWithContext(&model.Task{Properties: model.Properties{UpstreamModelName: "image"}},
...
func TestTaskAdaptorChainsDoubaoThroughNewAPIUpstream(t *testing.T) {
	...
	queryResp, err := adaptor.FetchTaskWithContext(gateway.URL, "sk-gateway", task, "")
	require.NoError(t, err)
	queryBody, err := io.ReadAll(queryResp.Body)
	require.NoError(t, err)
	require.NoError(t, queryResp.Body.Close())
	result, err := adaptor.ParseTaskResultWithContext(task, queryResp, queryBody)
	require.NoError(t, err)
	assert.Equal(t, "SUCCESS", result.Status)
