...
func TestUpstreamContextsUseGatewayBearerOnNewAPIChannels(t *testing.T) {
	...
	testCases := []struct {
		...
		{"api key plugin on a New API channel", "", constant.ChannelTypeNewAPI, "sk-gateway", "new_api", "Bearer sk-gateway", "sk-gateway"},
		{"api key plugin on a Task Plugin channel", "", constant.ChannelTypeTaskPlugin, "vendor-key", "vendor", "vendor-key", "vendor-key"},
		{"api key plugin on a legacy channel", "", 1002, "vendor-key", "vendor", "vendor-key", "vendor-key"},
		{"oauth2_jwt plugin on a New API channel", `,auth:{type:"oauth2_jwt"}`, constant.ChannelTypeNewAPI, "sk-gateway", "new_api", "Bearer sk-gateway", "sk-gateway"},
		{"oauth2_jwt plugin on a Task Plugin channel", `,auth:{type:"oauth2_jwt"}`, constant.ChannelTypeTaskPlugin, string(credentials), "vendor", "Bearer access-token", nil},
	}
