...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		{"currency below limit", operation_setting.QuotaDisplayTypeUSD, 4294, 2_147_000_000, false},
		{"currency above limit", operation_setting.QuotaDisplayTypeUSD, 4295, 0, true},
		{"token preserves settlement truncation", operation_setting.QuotaDisplayTypeTokens, common.MaxQuota, 2_147_000_000, false},
		{"token above settlement limit", operation_setting.QuotaDisplayTypeTokens, 2_147_500_000, 0, true},
	}
...
func TestValidateTopUpQuotaReturnsMaximumAmount(t *testing.T) {
	...
	maxAmount := decimal.NewFromInt(common.MaxQuota - 1).Div(decimal.NewFromFloat(common.QuotaPerUnit)).Floor().IntPart()
...
func TestRequestAmountRejectsTopUpThatCannotBeSettled(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/user/amount", strings.NewReader(`{"amount":4295}`))
...
func TestRequestAmountRejectsTopUpThatWouldOverflowWallet(t *testing.T) {
	...
	require.NoError(t, model.DB.Create(&model.User{Id: 42, Username: "topup_capacity_user", Quota: 1_000_000, Status: common.UserStatusEnabled}).Error)
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Set("id", 42)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/user/amount", strings.NewReader(`{"amount":4294}`))
	ctx.Request.Header.Set("Content-Type", "application/json")
	RequestAmount(ctx)
	assert.Equal(t, http.StatusOK, recorder.Code)
