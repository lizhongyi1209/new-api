...
func tasksToDto(tasks []*model.Task, fillUser bool, viewerRole int) []*dto.TaskDto {
	...
	for _, task := range tasks {
		...
		if _, loaded := channelTypes[task.ChannelId]; loaded {
			continue
		}
		channelTypes[task.ChannelId] = 0
		if channel, err := model.CacheGetChannel(task.ChannelId); err == nil && channel != nil {
			channelTypes[task.ChannelId] = channel.Type
		}
	}
	if fillUser {
	...
	for i, task := range tasks {
		...
		item := relay.TaskModel2Dto(task, false)
		if channelType := channelTypes[task.ChannelId]; channelType > 0 {
			item.ChannelType = channelType
			item.ChannelTypeName = constant.GetChannelTypeName(channelType)
		}
