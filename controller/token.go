...
type tokenBatchGroupRequest struct {
	Ids             []int    `json:"ids"`
...
func UpdateTokenBatchGroup(c *gin.Context) {
	request := tokenBatchGroupRequest{}
	...
	token := model.Token{Group: request.Group, CrossGroupRetry: request.CrossGroupRetry}
	if request.Group == "auto" {
		if !setTokenAutoGroups(c, &token, request.AutoGroups) {
			return
		}
	} else {
		token.CrossGroupRetry = false
	...
	count, err := model.BatchUpdateTokensGroup(
		request.Ids,
		c.GetInt("id"),
		token.Group,
		token.AutoGroups,
