...
func TestTaskPluginSourceAboveMySQLTextLimitRoundTrips(t *testing.T) {
	...
	require.NoError(t, err)
	source += "\n// " + strings.Repeat("source-padding", 1500)
	...
	plugin := model.TaskPlugin{Key: "alibaba", APIVersion: 1, Version: "roundtrip", Source: source, SourceHash: "hash", Enabled: true}
	require.NoError(t, model.SaveTaskPlugin(&plugin))
	// Re-saving the same version attaches the icon through the update path.
	withIcon := model.TaskPlugin{Key: "alibaba", APIVersion: 1, Version: "roundtrip", Source: source, SourceHash: "hash", Icon: icon, Enabled: true}
	...
	// The sync snapshot includes the active source used to compile the runtime.
	...
	assert.Equal(t, source, snapshot.Plugins[0].Source)
	assert.Equal(t, icon, snapshot.Plugins[0].Icon)
}
