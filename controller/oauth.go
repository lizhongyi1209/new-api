...
var (
	errAffiliateCodeRequired = errors.New("affiliate code is required")
	errAffiliateCodeInvalid  = errors.New("affiliate code is invalid")
)
...
func handleOAuthLogin(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, flow *model.AuthFlow) {
	...
	if err != nil {
		if errors.Is(err, errAffiliateCodeRequired) {
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string) (*model.User, error) {
	...
	// A legacy GitHub login name is not proof of ownership of the numeric
	// provider account. Require a separate verified migration flow.
	if legacyID, ok := oauthUser.Extra["legacy_id"].(string); ok && legacyID != "" {
		if provider.IsUserIDTaken(legacyID) {
			return nil, fmt.Errorf("legacy GitHub binding requires verification")
		}
	}
	...
	// This deployment requires a valid invitation for every new account,
	// including accounts first created through an OAuth provider.
	inviterId, err := resolveRequiredInviter(affiliateCode)
	if err != nil {
		return nil, err
	}
...
func resolveRequiredInviter(affiliateCode string) (int, error) {
	affiliateCode = strings.TrimSpace(affiliateCode)
	if affiliateCode == "" {
		return 0, errAffiliateCodeRequired
	}
