...
func getMaxTopUpAmount() int64 {
	...
	quotaPerUnit := decimal.NewFromFloat(common.QuotaPerUnit)
	maxStoredAmount := decimal.NewFromInt(common.MaxQuota - 1).Div(quotaPerUnit).Floor()
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		return maxStoredAmount.Add(decimal.NewFromInt(1)).
			Mul(quotaPerUnit).Ceil().Sub(decimal.NewFromInt(1)).IntPart()
	}
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if rechargeErr != nil {
			switch {
			case errors.Is(rechargeErr, model.ErrTopUpNotFound):
			...
			case errors.Is(rechargeErr, model.ErrPaymentMethodMismatch):
			...
			case errors.Is(rechargeErr, model.ErrTopUpStatusInvalid):
			...
			default:
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值处理失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), rechargeErr.Error()))
			}
			if _, writeErr := c.Writer.Write([]byte("fail")); writeErr != nil {
