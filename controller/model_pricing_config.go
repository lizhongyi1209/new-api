...
func UpdateTaskPluginPricingConfig(c *gin.Context) {
	var change model.TaskPluginPricingChange
	if err := common.DecodeJson(c.Request.Body, &change); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": err.Error()})
		return
	}
	if err := model.UpdateTaskPluginPricing(change); err != nil {
		status := http.StatusBadRequest
		if errors.Is(err, model.ErrModelPricingConflict) {
			status = http.StatusConflict
		}
	...
	recordManageAudit(c, "task_plugin.pricing.update", map[string]interface{}{
		"plugin_key": change.PluginKey, "model_name": change.ModelName, "reset": change.Reset,
	})
	common.ApiSuccess(c, gin.H{"plugin_key": change.PluginKey, "model_name": change.ModelName})
}
