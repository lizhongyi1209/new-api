...
`
	...
	require.NoError(t, err)
	t.Cleanup(func() { _ = pluginruntime.DefaultRegistry.Unregister("generic-settlement") })
	for index, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ch.SetSetting(kitdto.ChannelSettings{TaskPluginKey: plugin.Meta.Key})
			...
			if tc.status == "SUCCESS" {
				...
				require.NoError(t, common.Unmarshal(stored.PrivateData.BillingContext.TieredSnapshot, &snapshot))
				assert.Equal(t, tc.count, snapshot.UsageFacts["units"])
			}
