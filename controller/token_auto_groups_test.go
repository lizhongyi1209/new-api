...
// TestUpdateTokenBatchGroupAutoGroupsIgnoresOtherUsersTokens guards the ownership
// scope of the batch group endpoint: a caller may only rewrite the group snapshot
// of tokens they own, even when foreign ids are smuggled into the same request.
func TestUpdateTokenBatchGroupAutoGroupsIgnoresOtherUsersTokens(t *testing.T) {
	configureTokenAutoGroupsTest(t, "5", `["default","vip"]`)
	owner := setupTokenAutoGroupsControllerTest(t)
	other := &model.User{
		Id:       202,
		Username: "token-auto-other",
		Password: "password",
		Group:    "default",
		Status:   common.UserStatusEnabled,
		AffCode:  "other-aff",
	...
	ownerToken := seedToken(t, model.DB, owner.Id, "owner-token", "owner-token-key")
	foreignToken := seedToken(t, model.DB, other.Id, "foreign-token", "foreign-token-key")

	request := map[string]any{
		"ids":               []int{ownerToken.Id, foreignToken.Id},
		"group":             "auto",
		"auto_groups":       []string{"vip"},
		"cross_group_retry": true,
	}
	ctx, recorder := newTokenAutoGroupsAuthenticatedContext(t, http.MethodPost, "/api/token/batch/group", request, owner.Id)
	UpdateTokenBatchGroup(ctx)
	...
	var updatedOwnerToken model.Token
	require.NoError(t, model.DB.First(&updatedOwnerToken, ownerToken.Id).Error)
	assert.Equal(t, "auto", updatedOwnerToken.Group)
	assert.JSONEq(t, `["vip"]`, updatedOwnerToken.AutoGroups)
	assert.True(t, updatedOwnerToken.CrossGroupRetry)

	var untouchedForeignToken model.Token
	require.NoError(t, model.DB.First(&untouchedForeignToken, foreignToken.Id).Error)
	assert.Equal(t, "default", untouchedForeignToken.Group)
	assert.Empty(t, untouchedForeignToken.AutoGroups)
	assert.False(t, untouchedForeignToken.CrossGroupRetry)
}
...
func TestUpdateTokenBatchGroupSupportsSingleCustomAutoGroupAndFixedCleanup(t *testing.T) {
	configureTokenAutoGroupsTest(t, "5", `["default","vip"]`)
	user := setupTokenAutoGroupsControllerTest(t)
	first := seedToken(t, model.DB, user.Id, "batch-first", "batch-first-key")
	second := seedToken(t, model.DB, user.Id, "batch-second", "batch-second-key")

	autoRequest := map[string]any{
		"ids":               []int{first.Id, second.Id},
		"group":             "auto",
		"auto_groups":       []string{"vip"},
		"cross_group_retry": true,
	}
	ctx, recorder := newTokenAutoGroupsAuthenticatedContext(t, http.MethodPost, "/api/token/batch/group", autoRequest, user.Id)
	UpdateTokenBatchGroup(ctx)
	response := decodeAPIResponse(t, recorder)
	require.True(t, response.Success, response.Message)

	var updated []model.Token
	require.NoError(t, model.DB.Where("id IN ?", []int{first.Id, second.Id}).Order("id").Find(&updated).Error)
	require.Len(t, updated, 2)
	for _, token := range updated {
		assert.Equal(t, "auto", token.Group)
		assert.JSONEq(t, `["vip"]`, token.AutoGroups)
		assert.True(t, token.CrossGroupRetry)
	}

	fixedRequest := map[string]any{
		"ids":               []int{first.Id, second.Id},
		"group":             "default",
		"auto_groups":       []string{"vip"},
		"cross_group_retry": true,
	}
	ctx, recorder = newTokenAutoGroupsAuthenticatedContext(t, http.MethodPost, "/api/token/batch/group", fixedRequest, user.Id)
	UpdateTokenBatchGroup(ctx)
	response = decodeAPIResponse(t, recorder)
	require.True(t, response.Success, response.Message)
