...
func classifyDashboardCredential(c *gin.Context) (*model.UserBase, service.AuthIdentity, dashboardCredentialKind, error) {
	raw, hasAuthorization := authorizationToken(c.GetHeader("Authorization"))
	if hasAuthorization {
...
func writeDashboardAuthError(c *gin.Context, err error) {
	...
	case errors.Is(err, service.ErrLoginSessionRevoked), errors.Is(err, gorm.ErrRecordNotFound):
	...
	case errors.Is(err, service.ErrAuthTokenInvalid):
...
func TokenOrUserAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		user, identity, kind, err := classifyDashboardCredential(c)
		if err != nil {
			writeDashboardAuthError(c, err)
			return
		}
		if kind == dashboardCredentialLegacy {
			setDashboardAuthContext(c, user, identity, false)
			c.Next()
			return
		}
