...
func attachQuotaSaturationToOther(other any, clamp *common.QuotaClamp) {
	...
	if scoped, ok := other.(*model.LogOther); ok {
		scoped.SetAdmin("quota_saturation", clamp.AuditMap())
		return
	}
	legacy, ok := other.(map[string]interface{})
	if !ok {
		return
	}
	adminInfo, ok := legacy["admin_info"].(map[string]interface{})
	if !ok || adminInfo == nil {
		adminInfo = map[string]interface{}{}
		legacy["admin_info"] = adminInfo
	}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo != nil && relayInfo.ResponsesTiming != nil {
		other.SetAdmin("responses_timing", relayInfo.ResponsesTiming)
	}
