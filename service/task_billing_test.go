...
func billingSnapshotJSON(t *testing.T, snapshot *billingexpr.BillingSnapshot) json.RawMessage {
	t.Helper()
...
func readBillingSnapshot(t *testing.T, data json.RawMessage) billingexpr.BillingSnapshot {
	t.Helper()
...
func TestLogTaskConsumptionRecordsKlingMotionControlSettings(t *testing.T) {
	...
	relayInfo.PriceData.AddOtherRatio("resolution", 1.75)
	relayInfo.PriceData.AddOtherRatio("seconds", 5)
...
func callLogTaskConsumption(t *testing.T, info *relaycommon.RelayInfo, task *model.Task) *model.Log {
	...
	LogTaskConsumption(ctx, info, task.TaskID, task)
	log := getLastLog(t)
...
func TestSettle_TieredFailureReturnsFalseForCallerRefund(t *testing.T) {
	...
	assert.False(t, settled)
	assert.Equal(t, preConsumed, task.Quota)
	assert.Equal(t, map[string]any{"seconds": float64(5), "clips": float64(2)}, readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).UsageFacts)
	assert.Equal(t, "base", readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).EstimatedTier)
	assert.Equal(t, initialQuota, getUserQuota(t, userID))
	assert.Equal(t, int64(0), countLogs(t))
}
...
func TestSettle_TieredSuccessStillRecomputes(t *testing.T) {
	...
	expression := `tier("base", u("seconds") + u("clips") * 10)`
	task := makeTask(userID, 0, preConsumed, 0, BillingSourceWallet, 0)
	task.PrivateData.SubmitLogID = seedConsumeLog(t, task, preConsumed, map[string]interface{}{})
	...
	assert.Equal(t, map[string]any{"seconds": float64(8), "clips": float64(2)}, readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).UsageFacts)
	assert.Equal(t, "base", readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).EstimatedTier)
...
func TestSettle_TieredUsageFactsMergeCompletionOverSubmission(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			assert.Equal(t, testCase.expectedFacts, readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).UsageFacts)
			assert.Equal(t, "base", readBillingSnapshot(t, task.PrivateData.BillingContext.TieredSnapshot).EstimatedTier)
