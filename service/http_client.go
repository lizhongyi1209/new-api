...
const maxResponseHeaderTimeoutSeconds = int(math.MaxInt64 / int64(time.Second))
...
func newRelayHTTPTransport() *http.Transport {
	...
	if seconds := common.RelayResponseHeaderTimeout; seconds > 0 {
		if seconds > maxResponseHeaderTimeoutSeconds {
			seconds = maxResponseHeaderTimeoutSeconds
		}
