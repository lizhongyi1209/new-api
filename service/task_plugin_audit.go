...
// AppendTaskPluginAuditFields adds the role-scoped plugin provenance to legacy
// log maps without replacing existing admin metadata such as quota clamps.
func AppendTaskPluginAuditFields(ctx *gin.Context, other map[string]interface{}) {
	if other == nil {
		return
	}
	audit := model.NewLogOther()
	AppendTaskPluginContextAuditInfo(ctx, audit)
	for _, scope := range []string{"admin_info", "root_info"} {
		fields, ok := audit.Snapshot()[scope].(map[string]any)
		if !ok {
			continue
		}
		current, ok := other[scope].(map[string]interface{})
		if !ok || current == nil {
