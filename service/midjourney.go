...
// PrepareMidjourneyTaskBilling stores a durable refund marker before the task
// is inserted, while leaving unbilled tasks explicitly clear.
...
// SettleMidjourneyTaskBilling charges only after the task exists and records
// exactly which accounting stages were applied for a later safe refund.
...
func RefundMidjourneyQuota(ctx context.Context, task *model.Midjourney, reason string) bool {
	...
	logOther.SetPublic("task_id", task.MjId)
	logOther.SetPublic("reason", reason)
	model.RecordTaskBillingLog(model.RecordTaskBillingLogParams{
		UserId: task.UserId, LogType: model.LogTypeRefund, ChannelId: billingChannelId,
		ModelName: CovertMjpActionToModelName(task.Action), Quota: quota, TokenId: task.TokenId,
		Other: logOther,
	})
	task.Quota = 0
