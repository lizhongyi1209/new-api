...
// before every upstream attempt, including channel retries and parameter
// overrides. The original request snapshot remains frozen for settlement.
func PrepareImageBillingForRequest(c *gin.Context, info *relaycommon.RelayInfo, count int, promptExtend bool) *types.NewAPIError {
	if count < 1 || count > dto.MaxImageN {
		return types.NewErrorWithStatusCode(
			fmt.Errorf("image_count must be an integer between 1 and %d", dto.MaxImageN),
	...
	if snap := info.TieredBillingSnapshot; snap != nil && snap.BillingMode == "tiered_expr" {
		// Token-only image expressions must not acquire an implicit quantity
		// multiplier. Quantity is applied only when image_count was captured.
		...
		cost, trace, runErr := billingexpr.RunExprByHashWithRequest(
			snap.ExprString,
			snap.ExprHash,
		...
		// Reset per-attempt ratios so an Ali failure cannot leak its count or
		// prompt-extension surcharge into a retry on another channel.
		...
		quota, err = common.QuotaFromFloatStrict(
			info.PriceData.ApplyOtherRatiosToFloat(base * info.PriceData.GroupRatioInfo.GroupRatio),
		)
	}
	if err != nil {
