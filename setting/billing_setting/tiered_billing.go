...
func GetBillingExprCopy() map[string]string {
	...
	for model := range builtinBillingExpr {
		...
		if expr, ok := GetBillingExpr(model); ok {
...
func smokeTestExpr(exprStr string) error {
	...
	if billingexpr.UsesUsagePricing(exprStr) {
		return fmt.Errorf("usage-derived billing requires a configured usage schema")
	}
	vectors := []billingexpr.TokenParams{
		{P: 0, C: 0, Len: 0},
		{P: 1000, C: 1000, Len: 1000},
		{P: 500, C: 200, Len: 1000, CR: 100, Img: 300, ImgCR: 100, ImgO: 200},
		{ImgCR: 1000},
...
// usage dimensions. The ordinary expression validator continues to reject u().
func SmokeTestTaskExpr(exprStr string, schema map[string]jsplugin.UsageFieldSchema) error {
	if _, err := billingexpr.CompileFromCache(exprStr); err != nil {
		return err
	}
	if len(schema) == 0 || !billingexpr.UsesUsagePricing(exprStr) {
		return fmt.Errorf("task plugin billing requires a declared usage schema and u() expression")
	}
	...
	paramPaths, err := billingexpr.ReferencedParams(exprStr)
	if err != nil {
		return err
	}
	if _, err := billingexpr.ReferencedHeaders(exprStr); err != nil {
		return err
	}
	params := make(map[string]any, len(paramPaths))
	for _, path := range paramPaths {
		params[path] = float64(1)
	}
	for _, usage := range taskUsageSmokeVectors(schema) {
		for _, request := range []billingexpr.RequestInput{{Params: params}, {
...
func taskUsageSmokeVectors(schema map[string]jsplugin.UsageFieldSchema) []map[string]any {
	...
	for _, name := range names {
		...
		dimensions = append(dimensions, usageSmokeDimension{name: name, values: []any{float64(0), float64(1), float64(limit)}})
	}
	...
	if usageSmokeCombinationCount(dimensions, maxTaskExprSmokeTests) > maxTaskExprSmokeTests && len(vectors) > 0 {
