...
var defaultModelRatio = map[string]float64{
	...
	// The MiniMax H3 family is settled by its task adaptor from provider-native
	// per-second/input pricing. These ratios only open the standard task billing
	// path and provide the base used for submit-time pre-consumption.
	"MiniMax-H3":     1,
	"MiniMax-H3-Max": 1,
	"MiniMax-H3-MAX": 1,
	...
	"kling-v3-motion-t":            1,
	"dreamina-seedance-2-0-260128": 1,
}
...
func GetDefaultPricingMaps() map[string]map[string]float64 {
	defaults := map[string]map[string]float64{
		...
		"VideoCompletionRatio": defaultVideoCompletionRatio,
	}
