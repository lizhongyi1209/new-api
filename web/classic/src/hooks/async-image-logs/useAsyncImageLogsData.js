...
export const useAsyncImageLogsData = () => {
  ...
  const [compactMode, setCompactMode] = useTableCompactMode('asyncImageLogs');
  ...
  const loadLogs = async (page = 1, size = pageSize) => {
    ...
    let url = isAdminUser
      ? `/api/task/?p=${page}&page_size=${size}&platform=async_image&channel_id=${channel_id}&task_id=${task_id}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`
      : `/api/task/self?p=${page}&page_size=${size}&platform=async_image&task_id=${task_id}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
  ...
  const handlePageSizeChange = async (size) => {
    localStorage.setItem('async-image-page-size', size + '');
  ...
  useEffect(() => {
    const localPageSize =
      parseInt(localStorage.getItem('async-image-page-size')) || ITEMS_PER_PAGE;
