...
export async function fetchUpstreamModels(
  ...
  const res = await api.get(
    ...
    channelActionConfig({ signal: options?.signal, disableDuplicate: Boolean(options?.signal) })
  )
  return res.data
}
...
export async function fetchModels(data: ChannelModelDiscoveryRequest, options?: { signal?: AbortSignal }): Promise<FetchModelsResponse> {
  const res = await api.post(
