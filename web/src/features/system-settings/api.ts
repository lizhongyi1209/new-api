...
export async function getR2PublicUploadClients() {
  const response = await api.get<R2PublicUploadClientsResponse>('/api/option/r2_public_upload_clients')
  return response.data
}

export async function updateR2PublicUploadClients(clients: R2PublicUploadClient[]) {
  const response = await api.put<UpdateOptionResponse>('/api/option/r2_public_upload_clients', clients)
  return response.data
}

export async function rotateR2PublicUploadSecret(clientId: string) {
  const response = await api.post<RotateR2PublicUploadSecretResponse>(
