...
import { readTimePricingTiers } from '../lib/time-price'
...
type DynamicPricingBreakdownProps = {
  ...
  requestRuleTraces?: { cond: string; multiplier: number; matched: boolean }[]
  /**
...
function isSettledPricingTier(
  tier: ParsedTier,
  label: string | null | undefined,
  unit: 'token' | 'request' | undefined,
  price: number | undefined
): boolean {
  if (!label || normalizeTierLabel(tier.label) !== normalizeTierLabel(label)) {
    return false
  }
  if (unit && (tier.billingUnit ?? 'token') !== unit) return false
  return unit !== 'request' || price === undefined || tier.fixedPrice === price
}

function hasTierPrice(
  tier: ParsedTier,
  field: string | null,
  _timePricing: boolean
): boolean {
  if (!field) return false
  const value = Number(tier[field])
  return tier[field] !== undefined && Number.isFinite(value) && value >= 0
}
...
export function DynamicPricingBreakdown({
  ...
  compact = false,
  currentTimePreview = false,
}: DynamicPricingBreakdownProps) {
  const { t, i18n } = useTranslation()
  const expr = billingExpr || ''
  const billingTime = useBillingTime(expr, currentTimePreview)
  ...
  const { tiers, ruleGroups, currentTiers, isTimePricing } = useMemo(() => {
    const split = splitBillingExprAndRequestRules(expr)
    const timePricing = readTimePricingTiers(
      split.billingExpr,
      billingTime === undefined ? undefined : new Date(billingTime)
    )
    const parsedTiers =
      timePricing?.tiers ?? parseTiersFromExpr(split.billingExpr)
    if (
      matchedBillingUnit === 'request' &&
      typeof matchedFixedPrice === 'number' &&
      Number.isFinite(matchedFixedPrice) &&
      matchedFixedPrice >= 0
    ) {
      const selected = parsedTiers.find(
        (tier) =>
          normalizeTierLabel(tier.label) ===
            normalizeTierLabel(matchedTierLabel ?? undefined) &&
          tier.billingUnit === 'request' &&
          tier.fixedPrice === matchedFixedPrice
      )
      if (!selected && matchedTierLabel) {
        parsedTiers.push({
          label: matchedTierLabel,
          conditions: [],
          requestPrice: 0,
          billingUnit: 'request',
          fixedPrice: matchedFixedPrice,
        })
      }
    }
    ...
    return {
      ...
      currentTiers: timePricing?.currentTiers ?? [],
  ...
  const describeTier = (tier: ParsedTier) =>
    tier.conditionText
  ...
  const visiblePriceFields = BILLING_PRICING_VARS.filter((v) => {
    ...
    if (hideCacheColumns && (v.group === 'cache' || v.key === 'img_cr')) {
      return false
    }
    return tiers.some((tier) => hasTierPrice(tier, v.field, isTimePricing))
  })
  const hasRequestPrices = tiers.some(
    (tier) =>
      tier.billingUnit === 'request' || Number(tier.requestPrice || 0) > 0
  )
  ...
  return (
    <section className={cn('min-w-0', !compact && 'py-3 sm:py-4')}>
      ...
      {currentTimePreview && isTimePricing && (
        <p className='text-muted-foreground mb-2 text-xs'>
          {currentTiers.length
            ? t('Current time period: {{tiers}}', {
                tiers: [
                  ...new Set(currentTiers.map((tier) => tier.label)),
                ].join(', '),
              })
            : t(
                'Current time period cannot be determined in this browser.'
              )}{' '}
          {t(
            'This is a time preview only. Usage and request conditions determine the final charge.'
          )}
        </p>
      )}
      {hasTiers && (
        <div className={cn(compact ? cn(hasRules && 'mb-2') : 'mb-3 sm:mb-4')}>
          ...
          <div className='space-y-1.5 sm:hidden'>
            {tiers.map((tier) => {
              const condSummary = describeTier(tier)
              const isMatched = isSettledPricingTier(
                tier,
                matchedTierLabel,
                matchedBillingUnit,
                matchedFixedPrice
              ...
              return (
                <div
                  key={`tier-mobile-${tier.label}-${tier.conditionText ?? ''}`}
                  className={cn(
                    ...
                    {hasRequestPrices && (
                      <div className='min-w-0'>
                        ...
                        >
                          ...
                          (tier.billingUnit === 'request' ||
                            Number(tier.requestPrice) > 0)
                            ? `${symbol}${(Number(tier.billingUnit === 'request' ? tier.fixedPrice : tier.requestPrice) * rate).toFixed(4)}`
                            : '-'}
                          {tier.billingUnit === 'request' &&
                            ` / ${t(tier.imageCount ? 'image' : 'request')}`}
                        </div>
                      </div>
                    )}
                    {visiblePriceFields.map((v) => {
                      ...
                      return (
                        <div key={v.field} className='min-w-0'>
                          ...
                          >
                            {hasTierPrice(tier, v.field, isTimePricing)
          ...
          <StaticDataTable
            ...
            data={tiers}
            getRowKey={(tier, index) => `tier-${index}-${tier.label}`}
            ...
            columns={[
                ...
                ? [
                    {
                      ...
                      cell: (tier: ParsedTier) => {
                        ...
                        return Number.isFinite(value) &&
                          (tier.billingUnit === 'request' || value > 0) ? (
              ...
              ...visiblePriceFields.map((v, index) => ({
                ...
                cell: (tier: ParsedTier) => {
                  ...
                  return hasTierPrice(tier, v.field, isTimePricing) ? (
                    <span className={cn(!compact && 'font-semibold')}>
                      {`${symbol}${(value * rate).toFixed(4)}`}
                    </span>
                  ) : (
      ...
      {requestRuleTraces?.map((rule) => (
        <p
          key={`${rule.cond}-${rule.multiplier}`}
          className='text-muted-foreground text-xs'
        >
          {formatBillingCondition(rule.cond, t, i18n.language) ?? rule.cond} · ×
          {rule.multiplier} · {t(rule.matched ? 'Matched' : 'Not matched')}
        </p>
      ))}
      {hasRules && !requestRuleTraces && (
        <div>
          <div
            className={
              compact
