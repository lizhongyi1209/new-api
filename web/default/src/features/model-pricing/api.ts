...
export type ModelPricingEntry = {
  ...
  usage_variants?: BillingPluginVariant[]
}

export type ModelPricingDescription = {
  effective: PricingValues
  cache_write_mode?: 'none' | 'standard' | 'claude_ttl'
  billing_details?: {
...
}): Promise<ModelPricingDescription> {
...
export type TaskPluginPricingChange = {
  plugin_key: string
  model_name: string
  expected_version: string
  billing_expr: string
...
export async function saveTaskPluginPricing(change: TaskPluginPricingChange) {
  const response = await api.patch('/api/option/task_plugin_pricing', change)
  if (!response.data.success) {
    throw createServerError(response.data, t('Failed to save model pricing'))
  }
}
