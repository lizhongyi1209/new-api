...
export function PrefillGroupFormDrawer({
  ...
  const handleSubmit = async (values: PrefillGroupFormValues) => {
    ...
    const payload: {
      name: string
      type: PrefillGroupFormValues['type']
      description: string
      items: string | string[]
    } = {
    ...
    if (values.type === 'endpoint') {
      payload.items = typeof values.items === 'string' ? values.items : ''
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        handleServerError(response, 'Operation failed')
      }
    } catch (err: unknown) {
      handleServerError(err, (err as Error)?.message || 'Operation failed')
    } finally {
      setIsSaving(false)
    }
  }
