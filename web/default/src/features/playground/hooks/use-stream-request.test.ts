...
function deferred<T>() {
  let resolve: (value: T) => void = () => {
    throw new Error('Deferred request unavailable')
  }
...
describe('latest-wins stream request coordination', () => {
  ...
  test('closes the previous source and ignores all of its later events', async () => {
    ...
    sources[0]?.emit(
      'error',
      JSON.stringify({ error: { message: 'stale failure' } })
    )
...
test('reports source construction failure without leaving generation active', async () => {
  ...
  const controller = createStreamRequestController({
    ...
    setStreaming: (state) => states.push(state),
  })
  await controller.send(payload, { ...noopCallbacks, onError })
