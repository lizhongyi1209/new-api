...
function GroupOverrideRules({
  ...
  // Base group ratios keyed by name (used to auto-compute final inter-group ratios)
  const baseGroupRatioMap = useMemo(() => {
    const map: Record<string, number> = {}
    for (const entry of registry) map[entry.name] = entry.ratio
    return map
  }, [registry])

  // User-selectable groups (target candidates for quick add)
  ...
  const handleQuickAddOpen = useCallback((userGroup: string) => {
