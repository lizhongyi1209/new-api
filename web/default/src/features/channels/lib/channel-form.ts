...
export function normalizeHttpProtocol(
  ...
  const normalized = String(value || '')
    .trim()
    .toLowerCase()
  return normalized === HTTP_PROTOCOL_HTTP1
    ? HTTP_PROTOCOL_HTTP1
    : HTTP_PROTOCOL_AUTO
}
...
export function normalizeHttp2ConnectionShards(
  ...
  if (value < 1) return 1
  if (value > MAX_HTTP2_CONNECTION_SHARDS) return MAX_HTTP2_CONNECTION_SHARDS
  return value
}
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Field passthrough controls:
  // - OpenAI, Anthropic, Codex, Advanced Custom, Sub2API, New API: allow_service_tier
  // - OpenAI request fields: OpenAI, Codex, Advanced Custom, Sub2API, New API
  // - Claude request fields: Anthropic, Advanced Custom, Sub2API, New API
  ...
  if (formData.image_output_strategy) {
