...
function QuotaLogPreview(props: { log: UsageLog }) {
  const table = useReactTable({
    data: [props.log],
    columns: useCommonLogsColumns(false),
...
describe("quota adjustment log localization", () => {
  ...
  test.each(cases)(
    ...
    async (scenario) => {
      ...
      // The compact table and dialog summary both stay localized, while the
      // dialog additionally exposes individually readable operation fields.
      ...
      expect(dialog.getByText("Quota adjustment details")).toBeVisible();
      expect(dialog.getByText("Requested quota")).toBeVisible();
      ...
      expect(dialog.getByText("额度调整详情")).toBeVisible();
      expect(dialog.getByText("请求数额")).toBeVisible();
