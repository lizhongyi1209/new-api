...
func TestReferralProgramSummaryAndUsers(t *testing.T) {
	...
	assert.Equal(t, "root-referrer", summary.ReferralCode)
	assert.InDelta(t, 2.4, summary.TotalRevenue, 0.0001)
	assert.Equal(t, ReferralLevelSummary{Level: 1, Count: 2, TotalTopUp: 15, RewardRate: 10, Revenue: 1.5}, summary.Levels[0])
	assert.Equal(t, ReferralLevelSummary{Level: 2, Count: 1, TotalTopUp: 20, RewardRate: 3, Revenue: 0.6}, summary.Levels[1])
	assert.Equal(t, ReferralLevelSummary{Level: 3, Count: 1, TotalTopUp: 30, RewardRate: 1, Revenue: 0.3}, summary.Levels[2])

	levelOneUsers, err := GetReferralUsers(root.Id, 1)
	require.NoError(t, err)
	require.Len(t, levelOneUsers, 2)
	assert.Equal(t, ReferralUser{Username: "level-1-b", CreatedAt: 210, TotalTopUp: 0, Revenue: 0}, levelOneUsers[0])
	assert.Equal(t, ReferralUser{Username: "level-1-a", CreatedAt: 200, TotalTopUp: 15, Revenue: 1.5}, levelOneUsers[1])
