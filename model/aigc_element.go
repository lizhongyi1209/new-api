...
// GetUserAigcElements lists a single user's elements, newest first.
func GetUserAigcElements(userId, startIdx, num int) ([]*AigcElement, int64, error) {
	var elements []*AigcElement
	var total int64
	if err := DB.Model(&AigcElement{}).Where("user_id = ?", userId).Count(&total).Error; err != nil {
		return nil, 0, err
	}
	err := DB.Where("user_id = ?", userId).Order("id desc").Limit(num).Offset(startIdx).Find(&elements).Error
...
func GetAllAigcElements(startIdx, num int) ([]*AigcElement, int64, error) {
	var elements []*AigcElement
	var total int64
	if err := DB.Model(&AigcElement{}).Count(&total).Error; err != nil {
		return nil, 0, err
	}
	if err := DB.Order("id desc").Limit(num).Offset(startIdx).Find(&elements).Error; err != nil {
		return nil, 0, err
	}
