...
func (m mysqlSchemaMigrator) MigrateColumn(value any, field *schema.Field, column gorm.ColumnType) error {
	storedType := strings.ToLower(column.DatabaseTypeName())
	if (storedType == "mediumtext" || storedType == "longtext") && strings.EqualFold(m.Migrator.Dialector.DataTypeOf(field), storedType) {
		// MySQL reports the capacity of MEDIUMTEXT/LONGTEXT as its length,
		// whereas the model size is the minimum capacity it needs. Both
		// values describe the same storage type, so keep GORM's checks for
		// nullability, defaults, comments, and uniqueness without a rebuild.
		column = mysqlTextColumnType{migrationColumnType: column, size: int64(field.Size)}
	}
	...
	if precision, scale, hasSize := column.DecimalSize(); hasSize {
		declaredType := strings.ToLower(m.Migrator.Dialector.DataTypeOf(field))
		sameSize := strings.HasPrefix(declaredType, fmt.Sprintf("decimal(%d,%d)", precision, scale))
		nullable, hasNullable := column.Nullable()
		comment, hasComment := column.Comment()
		if sameSize && hasNullable && nullable != field.NotNull && (!hasComment || comment == field.Comment) {
			// GORM compares the padded database default (0.000000) with the
			// literal model default (0) and otherwise rebuilds this column on
			// every startup. All observable column properties already match.
...
type mysqlTextColumnType struct {
	migrationColumnType
	size int64
}

func (column mysqlTextColumnType) Length() (int64, bool) { return column.size, true }
