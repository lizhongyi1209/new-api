...
type migrationMediumTextV1 struct {
	ID     int    `gorm:"primaryKey"`
	Source string `gorm:"size:8388608;not null"`
}

type migrationMediumTextV2 struct {
	ID     int    `gorm:"primaryKey"`
	Source string `gorm:"size:20000000;not null"`
}
...
func TestMigrationSchemaStability(t *testing.T) {
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			if dialect == "mysql" {
				t.Run("mediumtext_capacity", func(t *testing.T) {
					const table = "migration_mediumtext_test"
					t.Cleanup(func() { _ = db.Migrator().DropTable(table) })
					require.NoError(t, db.Table(table).AutoMigrate(&migrationMediumTextV1{}))
					require.NoError(t, db.Table(table).Create(&migrationMediumTextV1{ID: 1, Source: "plugin source"}).Error)
					recorder.reset()
					require.NoError(t, db.Table(table).AutoMigrate(&migrationMediumTextV1{}))
					assert.Empty(t, recorder.schemaMutations())
					require.NoError(t, db.Table(table).AutoMigrate(&migrationMediumTextV2{}))
					var saved migrationMediumTextV2
					require.NoError(t, db.Table(table).First(&saved, 1).Error)
					assert.Equal(t, "plugin source", saved.Source)
					recorder.reset()
					require.NoError(t, db.Table(table).AutoMigrate(&migrationMediumTextV2{}))
					assert.Empty(t, recorder.schemaMutations())
				})
