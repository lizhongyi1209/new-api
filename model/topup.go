...
// settlement path repeats the invariant atomically because the wallet balance
// can change after checkout creation.
...
// creditTopUpQuota keeps the ceiling check and increment in one UPDATE so two
// concurrent callbacks cannot independently authorize an overflowing credit.
...
// RechargeEpay completes the order and credits quota in one transaction. The
// database row lock and status check make duplicate callbacks idempotent even
