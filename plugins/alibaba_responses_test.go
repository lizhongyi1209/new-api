...
func TestAlibabaWanCompletionFactsAndArtifacts(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			result, err := adaptor.ParseTaskResultWithContext(task, &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)}, encoded)
			require.NoError(t, err)
...
func TestAlibabaImageResults(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := adaptor.ParseTaskResultWithContext(task, &http.Response{StatusCode: http.StatusOK}, encoded)
			require.NoError(t, err)
			assert.Equal(t, "SUCCESS", result.Status)
	...
	t.Run("interleaved text without images has zero image usage", func(t *testing.T) {
		...
		require.NoError(t, err)
		adaptor := taskplugin.New(plugin)
		result, err := adaptor.ParseTaskResultWithContext(&model.Task{Properties: model.Properties{UpstreamModelName: "wan2.6-image"}}, &http.Response{StatusCode: http.StatusOK}, encoded)
		require.NoError(t, err)
		assert.Equal(t, "SUCCESS", result.Status)
