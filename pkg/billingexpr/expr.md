...
`tier("request", fixed(0.025))` means USD 0.025 per successful request. Under v1 the engine returns `0.025 * 1_000_000`; the existing `/1_000_000 * quota_per_unit * group_ratio` conversion therefore remains unchanged. A literal zero fixed price stays zero and uses the request billing unit. Fixed amounts must be finite, non-negative numeric literals. Each execution must select one token or fixed pricing leaf; adding both charges, multiplying a fixed amount by token usage, and invalid amounts in unselected branches are rejected before optimization. Conditional tiers, validated `image_count` and supported request-dependent multipliers are allowed. Realtime currently rejects expressions containing fixed pricing, including unselected branches.

`img_cr` separately prices image cache reads only when referenced by the expression. OpenAI `cached_tokens_details.image_tokens` must be explicitly present, non-negative, no greater than either total cache or image tokens, and consistent with the prompt/cache/image union and any reported text/audio cache details. Valid overlap is removed from `cr` and `img`; each separately priced category is removed from `p` once. `len` keeps the original input total. Missing or invalid overlap retains aggregate-cache normalization, and invalid details are logged. Existing expressions without `img_cr` and the local Claude cache-creation split retain their previous behavior.

Snapshots add optional estimated billing-unit/fixed-price fields. Historical snapshots without them continue using the existing v1 token scale and do not acquire an implicit image count. Logs can record the actual unit, fixed amount, request rules and normalized `billing_tokens` for explicit image-cache expressions. Ordinary model pricing still rejects `u(...)`. Task-plugin pricing is stored separately by plugin key and model name in the `TaskPluginPricing` option, validated against the active plugin's usage schema and saved with an optimistic version. Submission selects only that scoped expression and records estimated usage facts, referenced scalar `param()` fields, non-credential literal headers and submission time in a billing snapshot. Successful polling reuses those frozen conditions with validated completion facts. Dynamic `param()`/`header()` names are rejected for these asynchronous tasks. Focused price-isolation and frozen-snapshot settlement tests pass; end-to-end plugin execution and production deployment remain unverified.

Implementation status (2026-09-14): engine, synchronous images and two OpenAI asynchronous image entrances have isolated request/ledger evidence. Async submission records unit/count fields and a scalar request projection; successful explicit zero results release wallet/subscription and token reservations. New task snapshots use shared opt-in normalization and input-only `len`; historical snapshots without estimated units retain their previous task normalization. Referenced literal non-credential headers are now frozen across async submission/settlement; dynamic or credential header references are rejected before pre-consume, including unselected branches. Native Gemini freezes canonical generation parameters without content/schema/media, bounds candidateCount and shared output tokens, and its workers preserve prompt/cache/candidate modality facts. Three Gemini image entrances and header conditions have isolated actual-request ledger evidence. Audio request-unit/zero-usage behavior, ordinary/streaming/compact Responses output details and final task saturation accounting/admin-only completion audit have isolated request and regression evidence. Batch 5B-1 is locally complete; see `docs/reviews/ui-batch-5b1-validation-2026-09-14.md`. See `docs/reviews/ui-batch-5b1-gemini-progress-2026-09-14.md`. Async evidence is recorded in `docs/reviews/ui-batch-5b1-async-progress-2026-09-14.md`. Batch 5B-2 now enables fixed/img_cr in the bounded frontend parser, adds lossless condition editing/request simulation and readonly pricing conversion/preview. Batch 5B-3 adds settled-unit/count/cache facts to historical log details, fixed/image/cache prices to the model square and built-in expressions with explicit legacy-price/zero/mode precedence; readonly default exposure does not persist options. See `docs/reviews/ui-batch-5b3-validation-2026-09-14.md`. Explicit save remains separate from applying a conversion to the draft; usage-derived u(...) remains gated until batch 7. See `docs/reviews/ui-batch-5b2-validation-2026-09-14.md`. See `docs/reviews/ui-batch-5b1-progress-2026-09-14.md`; this is not a production rollout or acceptance of the full batch 5B/editor/plugin scope.
