...
func TestSummaryHourlyWindowMergesTrafficAndPreservesLegacyFields(t *testing.T) {
	...
	assert.Equal(t, hour-23*3600, result.WindowStart)
	assert.Equal(t, end, result.WindowEnd)
	summary := result.Models[0]
	assert.Equal(t, int64(25), summary.RequestCount)
	assert.Equal(t, 48.0, summary.SuccessRate)
	assert.Equal(t, []SuccessRatePoint{{Ts: hour - 23*3600, SuccessRate: 100}, {Ts: hour - 3600, SuccessRate: 45}, {Ts: hour, SuccessRate: 50}}, summary.RecentSuccessSeries)
	...
	assert.Equal(t, []SuccessRatePoint{{Ts: hour, SuccessRate: 50}}, short.Models[0].RecentSuccessSeries)
	...
	assert.Equal(t, hour-3600, next.Models[0].RecentSuccessSeries[0].Ts)
	...
	assert.Contains(t, string(encoded), `"recent_success_series"`)
}
