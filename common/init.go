...
func InitEnv() {
	...
	// 分销子站：是否将渠道写操作收紧到仅 root 可用
	LockChannelToRoot = GetEnvOrDefaultBool("LOCK_CHANNEL_TO_ROOT", false)
...
func initUserSessionSettings() {
	...
	if int64(UserSessionRevokedRetentionDays) > math.MaxInt64/secondsPerDay {
		SysError(fmt.Sprintf("USER_SESSION_REVOKED_RETENTION_DAYS is too large, using default value: %d", DefaultUserSessionRevokedRetentionDays))
