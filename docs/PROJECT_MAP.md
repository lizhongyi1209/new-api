...
| Signed downstream R2 upload | `POST /v1/storage/public/presign` | Route: `router/relay-router.go`; signature/origin/rate-limit checks: `controller/storage_public.go`; R2 presign: `service/storage.go`; admin whitelist UI: `web/src/features/system-settings/security/r2-public-upload-section.tsx` |
