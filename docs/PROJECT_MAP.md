...
| Seedance unified asset creation from a public URL | `POST /v1/seedance/assets`; status: `GET /v1/seedance/assets/:asset_id?type=hc` | Routes: `router/video-router.go`; stable request dispatcher and upstream proxy: `controller/seedance_asset_proxy.go`; downstream integration guide: `docs/seedance-downstream-integration.md` |
