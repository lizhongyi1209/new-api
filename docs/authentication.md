...
面板主鉴权采用短期 Access Token、HttpOnly Refresh Cookie 与服务端登录会话控制面的组合。Default 与 Classic 前端的 API 请求都使用 Bearer/Refresh 协议，不要求 `New-Api-User` 请求头。为保持当前双前端迁移兼容，登录时仍可签发带 `auth_version` 的新 Gin session Cookie；它仅作为受控兼容路径，升级前没有版本标记的旧 Cookie 会被拒绝。
...
`User.AccessToken`（面板 PAT）继续支持 `Authorization: Bearer <pat>`，也兼容原有的单值 `Authorization: <pat>`。Bearer/PAT 主路径不再需要 `New-Api-User`；旧 PAT 本身无需重新生成。仅仍在迁移期使用兼容 Gin session 的旧式客户端可能继续携带该请求头。
...
- 升级前的旧 `session` Cookie 没有 `auth_version`，会被拒绝并要求用户重新登录。升级后新登录可同时获得带版本标记的兼容 Cookie，但 Default 与 Classic 的正常 API 请求均以 Bearer/Refresh 协议为主。
