...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/status/github-latest-release", middleware.AdminAuth(), controller.GetGitHubLatestRelease)
		...
		apiRouter.POST("/oauth/state", middleware.SessionCookieOriginGuard(), middleware.CriticalRateLimit(), middleware.DisableCache(), middleware.TryUserAuth(), anonymousRequestBodyLimit, controller.GenerateOAuthCode)
		...
		// Standard OAuth providers, including Telegram.
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.GET("/token/status", controller.GetAccessTokenStatus)
				selfRoute.POST("/token", middleware.CriticalRateLimit(), middleware.UserCriticalRateLimit("access-token"), controller.GenerateAccessToken)
				...
				selfRoute.GET("/referrals", controller.GetReferralProgramSummary)
				selfRoute.GET("/referrals/:level", controller.GetReferralUsers)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.PATCH("/task_plugin_pricing", controller.UpdateTaskPluginPricingConfig)
		...
		taskRoute := apiRouter.Group("/task")
		{
			...
			taskRoute.GET("/:task_id/audit", middleware.AdminAuth(), controller.GetTaskAudit)
